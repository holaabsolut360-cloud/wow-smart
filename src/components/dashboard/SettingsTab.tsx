...
export function SettingsTab({ company, setCompany }: SettingsTabProps) {
  return (
    <div id="settings" className="max-w-3xl">
      ...
      <div className="mb-8 p-6 bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col md:flex-row items-center gap-6">
      ...
      <form onSubmit={(e) => e.preventDefault()} className="p-8 rounded-2xl bg-white border border-slate-200 shadow-sm">
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="col-span-2">
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Horario de atención (Una línea por renglón)</label>
            <textarea 
              value={company?.hours || ''}
          ...
          <div className="col-span-2">
            <div className="flex items-center justify-between mb-4">
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider">Redes Sociales</label>
              <button 
                type="button" 
          ...
          <div className="col-span-2 pt-6 border-t border-slate-200">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="font-bold text-slate-900">Marketing & SEO</h3>
                <p className="text-sm text-slate-500">Mejora tu visibilidad y mide tu tráfico en tu catálogo público</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
              <div className="md:col-span-2">
                ...
                <input 
                  ...
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all text-slate-900"
                />
              ...
              <div className="md:col-span-2">
                ...
                <textarea 
                  ...
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all text-slate-900 h-24 resize-none"
                />
              </div>
          ...
          <div className="col-span-2 mt-4 pt-6 border-t border-slate-200">
            <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider mb-1">Pagos Anticipados (Opcional)</h3>
            <p className="text-sm text-slate-500 mb-6">Si los activas, tus clientes verán estos botones al ver un producto.</p>
          ...
          <div className="col-span-2 mt-4 pt-6 border-t border-slate-200">
            <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider mb-1">Horarios de Atención</h3>
            <p className="text-sm text-slate-500 mb-6">Configura cuándo estás disponible para recibir pedidos. Fuera de este horario, el catálogo mostrará un aviso de cerrado.</p>
                  ...
                  <div key={i} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 p-3 bg-slate-50 rounded-xl border border-slate-200">
                    <label className="flex items-center gap-2 cursor-pointer w-32">
                      <input 
                        ...
                        onChange={e => {
                          const currentSched = company.storeSchedule || [];
                          const newSched = [...currentSched];
                          if (!newSched[i]) {
                            newSched[i] = { isOpen: false, openTime: '09:00', closeTime: '18:00' };
                          }
                          newSched[i] = { ...newSched[i], isOpen: e.target.checked };
                    ...
                    {company.storeSchedule?.[i]?.isOpen ? (
                      <div className="flex items-center gap-2 flex-1">
                        <input 
                          type="time" 
                          value={company.storeSchedule[i]?.openTime || '09:00'}
                          onChange={e => {
                            const currentSched = company.storeSchedule || [];
                            const newSched = [...currentSched];
                            newSched[i] = { ...newSched[i], openTime: e.target.value };
                            setCompany(c => c ? {...c, storeSchedule: newSched} : null);
                          }}
                        ...
                        <input 
                          type="time" 
                          value={company.storeSchedule[i]?.closeTime || '18:00'}
                          onChange={e => {
                            const currentSched = company.storeSchedule || [];
                            const newSched = [...currentSched];
                            newSched[i] = { ...newSched[i], closeTime: e.target.value };
                            setCompany(c => c ? {...c, storeSchedule: newSched} : null);
                          }}
                          className="bg-white border border-slate-300 rounded-lg px-3 py-1.5 text-sm outline-none focus:border-indigo-500 font-medium"
                        />
